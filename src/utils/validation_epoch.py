...
def validation_epoch_fn(
    model, data_loader, loss_fn, device, description="", k_for_top_k_eval=None
):
    model.eval()
    ...
    with torch.no_grad():
        for inputs, targets in progress_bar:
            ...
            progress_bar.set_postfix(
                loss=f"{epoch_loss.avg:.4f}", acc=f"{100 * correct / total:.4f}"
            )
    ...
    if all_predictions_eval:
        ...
        if k_for_top_k_eval is not None:
            calculated_top_k_acc = top_k_accuracy_fn(
                all_predictions_tensor, all_labels_tensor, k=k_for_top_k_eval
            )
