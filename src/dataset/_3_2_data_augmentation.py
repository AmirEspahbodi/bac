...
def contextual_word_replacement_mlm(text, tokenizer, model, device, n_replacements=1, top_k=5, num_augnemtations=2):
    """
    ...
    for i in range(num_augnemtations):
        ...
        for token_idx_to_mask in token_indices:
            ...
            if current_original_token_in_loop in tokenizer.all_special_tokens or len(current_original_token_in_loop) <= 1:
                continue
            ...
            masked_text_for_model_input = tokenizer.convert_tokens_to_string(temp_masked_tokens)
            
            # Prepare input for the model
            inputs = tokenizer(masked_text_for_model_input, return_tensors="pt", truncation=True, max_length=512)
            ...
            predicted_token_ids = torch.topk(predictions[0, mask_token_index_in_ids], k=top_k, dim=-1).indices.tolist()
            ...
            for token_id in predicted_token_ids:
                replacement_token = tokenizer.decode([token_id]).strip() # Decode the token ID to a string
                ...
                if replacement_token and \
                replacement_token.lower() != current_original_token_in_loop.lower() and \
                not replacement_token.startswith("##") and \
                replacement_token not in tokenizer.all_special_tokens and \
                tokenizer.convert_tokens_to_ids(replacement_token) != tokenizer.unk_token_id:
                    
                    augmented_tokens[token_idx_to_mask] = replacement_token # Perform the replacement
                    replaced_count += 1
...
def contextual_word_replacement_augmentation(train_dataset, dataset_type: DatasetType):
    ...
    if tokenizer and model:
        ...
        print(f"Augmenting 'text_input' in train_dataset. Total rows: {len(train_dataset)}")
        
        # Define the number of words to attempt to replace per text entry
        num_replacements_per_text = 2  # Example: try to replace up to 2 words. Adjust as needed.
        ...
        for entry in tqdm(train_dataset.iterrows(), desc="Augmenting texts"):
            if pd.isna(entry[1]["text_input"]) or not isinstance(entry[1]["text_input"], str) or not entry[1]["text_input"].strip():
                continue
            try:
                ...
                print(f"Error during augmentation for text: '{str(entry[1]['text_input'])[:50]}...'. Error: {e}. Using original text.")
        ...
        if num_examples_to_show > 0:
            for i in range(num_examples_to_show):
                ...
                if original_text_example != augmented_text_example and not (pd.isna(original_text_example) and pd.isna(augmented_text_example)):
                    ...
                    print("Augmented:  (Not changed or error occurred during this specific augmentation)")
        else:
